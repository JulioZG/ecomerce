...
interface Variant { talla: string; color: string; stock: number; sku: string }
...
export default function NuevoProductoPage() {
  ...
  const [variants, setVariants] = useState<Variant[]>([
    { talla: "M", color: "Blanco", stock: 10, sku: "" },
  ])
  ...
  function addVariant() {
    setVariants((prev) => [...prev, { talla: "M", color: "", stock: 0, sku: "" }])
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  handleChange(e)
                  if (!form.slug) setForm((p) => ({ ...p, slug: autoSlug(e.target.value) }))
                }}
                required
                className="mt-1"
              />
              ...
              <div key={i} className="flex gap-2">
                <Input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      images: p.images.map((v, idx) => (idx === i ? e.target.value : v)),
                    }))
                  }
                  placeholder="https://..."
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-3">
            {variants.map((v, i) => (
              <div key={i} className="grid grid-cols-4 gap-2 p-3 bg-slate-50 rounded-lg items-end">
                <div>
                  <Label className="text-xs">Talla</Label>
                  <select
                    value={v.talla}
                    onChange={(e) => updateVariant(i, "talla", e.target.value)}
                    className="mt-1 w-full border border-slate-200 rounded-md px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <Input
                      ...
                      placeholder="CAM-R-M"
                      className="mt-1 text-sm h-8 font-mono"
                    />
                  </div>
                  {variants.length > 1 && (
                    <Button
